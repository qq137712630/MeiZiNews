 ...
 */
public abstract class MzituBaseModel implements CommonModel<List<ImgItem>> {
    ...
    public Subscription reImgSubscription(
            final OnModelListener<List<ImgItem>> listener,
            Observable<String> stringObservable
    ) {
        Observable observable = stringObservable.map(new Func1<String, List<ImgItem>>() {
            @Override
    ...
    protected MzituApi getMzituApi() {
        return MyStringRetrofit.getMyStringRetrofit().getCreate(MzituApi.class);
    }
}
