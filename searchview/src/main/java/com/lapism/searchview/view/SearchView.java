...
public class SearchView extends FrameLayout implements Filter.FilterListener, View.OnClickListener {
    ...
    protected int mVersion = SearchCodes.VERSION_TOOLBAR;
    protected int mStyle = SearchCodes.STYLE_TOOLBAR_CLASSIC;
    protected int ANIMATION_DURATION = 360;
    protected boolean mVoice = true;
    protected boolean mIsSearchOpen = false;
    protected float mIsSearchArrowHamburgerState = ArrowDrawable.STATE_HAMBURGER;
    protected String VOICE_SEARCH_TEXT = "Speak now";
    ...
    protected ArrowDrawable mSearchArrow;
    ...
    protected SearchMenuListener mSearchMenuListener;
    ...
    protected void initStyle(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    // protected -------------------------------------------------------------------------------------
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    protected void revealInAnimation() {
    ...
    protected void startFilter(CharSequence s) {
        if (mSearchAdapter != null) {
            (mSearchAdapter).getFilter().filter(s, this);
        }
    }

    protected void checkVoiceStatus(boolean status) {
